...
title('\textbf{Strategia Standard: Piano $\rightarrow$ Pericentro $\rightarrow$ Bitangente ''pa''}', 'Interpreter', 'latex', 'Color', 'k', 'FontSize', 15);
...
plotOrbit_ottimizz(a_i, e_i, i_f, OM_f, om_mid, mu, th_vec_plot, [0.494 0.184 0.556], '-', 1.5, 'Parcheggio (Post-Piano)');

% 3. Post-Cambio Pericentro (Ciano)
plotOrbit_ottimizz(a_i, e_i, i_f, OM_f, om_f, mu, th_vec_plot, [0.301 0.745 0.933], '-', 1.5, 'Parcheggio (Post-Pericentro)');

% 4. Trasferimento Bitangente 'pa' (Rosso Tratteggiato Spesso)
th_vec_trasf = linspace(th_start_b, th_end_b, 150);
plotOrbit_ottimizz(a_t, e_t, i_f, OM_f, om_f, mu, th_vec_trasf, [0.635 0.078 0.184], '--', 2.5, 'Trasferimento Bitangente (pa)');
