...
% non voglio orbite troppo piccole altrimenti la strategia perde di senso
...
for idx_a = 1:length(r_a_test_vec)
    ...
    for idx_p = 1:length(r_p_test_vec)
        ...
        % DA ATTIVARE SOLO PER ORBITE CIRCOLARI %

%         % --- FIX: SELEZIONE DEL NODO FUTURO ---
...
% end
        ...
        % coasting dal punto di cambio piano fino all'apocentro        
        ...
        [thi_arg, idx_thi_arg] = max(thi_fun);

        % coasting dal pericentro fino al punto di cambio anomalia del
        % pericentro
        dt_coast_periarg = TOF(a_f, e_f, 0, thi_arg, mu);

        % coasting al punto finale
        % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if dV_tot < min_costo
            min_costo = dV_tot;
            % FIX: Aggiunti i parametri angolari necessari per il plot 3D
...
fprintf('\n=======================================================\n');
fprintf('Risultati (Costi e TOF)\n');
fprintf('=======================================================\n');
fprintf('%.4f km/s | TOF: %.2f giorni\n', min_costo, opt.tof/86400);
...
plot3(arrayfun(@(th) get_c(opt.a_park,opt.e_park,i_f,OM_f,opt.om_plane,th,1), th_vec), arrayfun(@(th) get_c(opt.a_park,opt.e_park,i_f,OM_f,opt.om_plane,th,2), th_vec), arrayfun(@(th) get_c(opt.a_park,opt.e_park,i_f,OM_f,opt.om_plane,th,3), th_vec), 'Color', [0.4940 0.1840 0.5560], 'LineStyle', '-.', 'LineWidth', 1.5, 'DisplayName', 'Appoggio (Post-Plane)');
...
[r4,~] = par2car(opt.a_park, opt.e_park, i_f, OM_f, opt.om_plane, opt.th_peri, mu);
