...
% --- ESTRAZIONE RIGOROSA DATI PER TABELLA 5 MANOVRE ---
a_aux_opt = a_aux; e_aux_opt = 0;

% M1: Salita 1
dt1 = TOF(a_i, e_i, th_i, 0, mu);
t1 = dt1;
[dv1_1, dv1_2, dt_bit1] = bitangentTransfer(a_i, e_i, a_aux_opt, e_aux_opt, 'pa', mu);
a_t1 = (a_i*(1-e_i) + a_aux_opt)/2; e_t1 = abs(a_aux_opt - a_i*(1-e_i))/(a_aux_opt + a_i*(1-e_i));
...
% M3: Cambio Piano
[dv2_pl, om_tmp, th_plane_base] = changeOrbitalPlane(a_aux_opt, e_aux_opt, i_i, OM_i, om_i, i_f, OM_f, mu);
th_n1 = mod(th_plane_base, 2*pi); th_n2 = mod(th_plane_base + pi, 2*pi);
if th_n1 >= pi; th_plane_tmp = th_n1; else; th_plane_tmp = th_n2; end
dt3 = TOF(a_aux_opt, e_aux_opt, pi, th_plane_tmp, mu);
t3 = t2 + dt3;

% M4: Cambio Pericentro (Matematico)
dt4 = 0;
t4 = t3;
th_post_plane_1 = mod(th_plane_tmp + om_i - om_tmp, 2*pi);
th_post_plane_new = mod(th_post_plane_1 + om_tmp - om_f, 2*pi);

% M5: Discesa 1 
dt5 = TOF(a_aux_opt, e_aux_opt, th_post_plane_new, pi, mu);
t5 = t4 + dt5;
[dv4_1, dv4_2, dt_bit2] = bitangentTransfer(a_aux_opt, e_aux_opt, a_f, e_f, 'ap', mu);
a_t2 = (a_aux_opt + a_f*(1-e_f))/2; e_t2 = abs(a_aux_opt - a_f*(1-e_f))/(a_aux_opt + a_f*(1-e_f));
...
% Arrivo Finale
dt7 = TOF(a_f, e_f, 0, th_f, mu);
t7 = t6 + dt7;

% Calcolo DV cumulativi
sdv0 = 0;
sdv1 = abs(dv1_1);
sdv2 = sdv1 + abs(dv1_2);
sdv3 = sdv2 + abs(dv2_pl);
sdv4 = sdv3 + 0; 
sdv5 = sdv4 + abs(dv4_1);
sdv6 = sdv5 + abs(dv4_2);

fprintf('\n\n--- INCOLLAMI QUESTO OUTPUT ---\n');
fprintf('Init      : dt=-, t=0, a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=%.4f, dV=-, SdV=0\n', a_i, e_i, i_i, OM_i, om_i, th_i);
fprintf('Pre-M1    : dt=%.2f, t=%.2f, a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=0, dV=%.4f, SdV=%.4f\n', dt1, t1, a_i, e_i, i_i, OM_i, om_i, abs(dv1_1), sdv1);
fprintf('Post-M1   : a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=0\n', a_t1, e_t1, i_i, OM_i, om_i);
fprintf('Pre-M2    : dt=%.2f, t=%.2f, a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=pi, dV=%.4f, SdV=%.4f\n', dt2, t2, a_t1, e_t1, i_i, OM_i, om_i, abs(dv1_2), sdv2);
fprintf('Post-M2   : a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=pi\n', a_aux_opt, e_aux_opt, i_i, OM_i, om_i);
fprintf('Pre-M3    : dt=%.2f, t=%.2f, a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=%.4f, dV=%.4f, SdV=%.4f\n', dt3, t3, a_aux_opt, e_aux_opt, i_i, OM_i, om_i, th_plane_tmp, abs(dv2_pl), sdv3);
fprintf('Post-M3   : a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=%.4f\n', a_aux_opt, e_aux_opt, i_f, OM_f, om_tmp, th_post_plane_1);
fprintf('Pre-M4    : dt=%.2f, t=%.2f, a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=%.4f, dV=0.0000, SdV=%.4f\n', dt4, t4, a_aux_opt, e_aux_opt, i_f, OM_f, om_tmp, th_post_plane_1, sdv4);
fprintf('Post-M4   : a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=%.4f\n', a_aux_opt, e_aux_opt, i_f, OM_f, om_f, th_post_plane_new);
fprintf('Pre-M5    : dt=%.2f, t=%.2f, a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=pi, dV=%.4f, SdV=%.4f\n', dt5, t5, a_aux_opt, e_aux_opt, i_f, OM_f, om_f, abs(dv4_1), sdv5);
fprintf('Post-M5   : a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=pi\n', a_t2, e_t2, i_f, OM_f, om_f);
fprintf('Pre-M6    : dt=%.2f, t=%.2f, a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=0, dV=%.4f, SdV=%.4f\n', dt6, t6, a_t2, e_t2, i_f, OM_f, om_f, abs(dv4_2), sdv6);
fprintf('Post-M6   : a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=0\n', a_f, e_f, i_f, OM_f, om_f);
fprintf('Final     : dt=%.2f, t=%.2f, a=%.4f, e=%.4f, i=%.4f, OM=%.4f, om=%.4f, th=%.4f, dV=-, SdV=%.4f\n', dt7, t7, a_f, e_f, i_f, OM_f, om_f, th_f, sdv6);